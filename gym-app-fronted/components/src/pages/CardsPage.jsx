...
function buildClass(base, date, time) {
  return { ...base, date, time, schedule: fmtSchedule(date, time), enrolled:[], waitlist:[], attendance:[], promotionLog:[], id:`${Date.now()}-${Math.random().toString(36).slice(2,6)}`, linkToPage:"panels", linkLabel:"See Our Facilities →" };
}
...
function MembersModal({ gymClass, onClose }) {
  const { removeMember, toggleAttendance } = useClasses();
  ...
  return (
    <div className="modal-overlay"><div className="create-class-modal members-modal-wide">
      ...
      <div className="auth-tabs" style={{marginBottom:"1rem"}}>
        ...
        <button className={`auth-tab ${tab==="log"?"auth-tab--active":""}`} onClick={()=>setTab("log")}>
          Promotion Log {gymClass.promotionLog.length>0&&<span className="log-badge">{gymClass.promotionLog.length}</span>}
        </button>
      </div>
      {tab==="members" && <>
        <div className="members-section">
          <h4 className="members-heading">Enrolled <span className="members-count">{gymClass.enrolled.length}/{gymClass.capacity}</span><span className="attendance-legend">☑ = Attended</span></h4>
          {gymClass.enrolled.length===0 ? <p className="members-empty">No enrolled members yet.</p> : (
            <ul className="members-list">
              {gymClass.enrolled.map(uid => {
                const att = gymClass.attendance.includes(uid);
                return (
                  <li key={uid} className="members-row">
                    <label className="attendance-label">
                      <input type="checkbox" checked={att} onChange={()=>toggleAttendance(gymClass.id,uid)} />
                      <span className={`member-id ${att?"member-attended":""}`}>Member #{uid}</span>
                      {att && <span className="attended-tag">✓ Attended</span>}
                    </label>
                    <button className="remove-btn" onClick={()=>removeMember(gymClass.id,uid)}>Remove</button>
                  </li>
                );
              })}
            </ul>
          )}
          {gymClass.enrolled.length>0 && <p className="attendance-summary">Attendance: {gymClass.attendance.length} / {gymClass.enrolled.length} marked present</p>}
        </div>
        <div className="members-section">
          <h4 className="members-heading">Waitlist <span className="members-count">{gymClass.waitlist.length}</span></h4>
          {gymClass.waitlist.length===0 ? <p className="members-empty">No one on the waitlist.</p> : (
            <ul className="members-list">
              {gymClass.waitlist.map((uid,i)=>(
                <li key={uid} className="members-row"><span className="member-id">#{i+1} — Member #{uid}</span></li>
              ))}
            </ul>
          )}
        </div>
      ...
      {tab==="log" && (
        <div className="members-section">
          <h4 className="members-heading">Promotion History</h4>
          {gymClass.promotionLog.length===0 ? <p className="members-empty">No promotions yet.</p> : (
            <ul className="members-list">
              {gymClass.promotionLog.map((e,i)=>(
                <li key={i} className="members-row log-row">
                  <span className="log-entry">🎉 Member #{e.uid} was promoted from waitlist</span>
...
function TrainerCard({ gymClass, setCurrentPage, onEdit, onDelete, onViewMembers }) {
  return (
    <article className="class-card class-card--animated">
      ...
      <CapacityBar enrolled={gymClass.enrolled.length} capacity={gymClass.capacity} waitlist={gymClass.waitlist.length} />
      {gymClass.enrolled.length>0 && <p className="card-attendance-hint">☑ {gymClass.attendance.length} / {gymClass.enrolled.length} attended</p>}
...
function MemberCard({ gymClass, setCurrentPage }) {
  ...
  const { user } = useAuth();
  const uid          = user?.id;
  const isEnrolled   = gymClass.enrolled.includes(uid);
  const isWaitlisted = gymClass.waitlist.includes(uid);
  const isFull       = gymClass.enrolled.length >= gymClass.capacity;
  const waitPos      = gymClass.waitlist.indexOf(uid)+1;
  return (
    <article className="class-card class-card--animated">
      <h3 className="card-title">{gymClass.title}</h3>
      <LevelBadge level={gymClass.level} />
      {getTerm(gymClass.date) && <span className="card-term">{getTerm(gymClass.date)}</span>}
      {gymClass.trainerName && <p className="card-trainer-name">👤 {gymClass.trainerName}</p>}
      <p className="card-description">{gymClass.description}</p>
      <p className="card-schedule"><strong>Schedule:</strong> {gymClass.schedule}</p>
      <CapacityBar enrolled={gymClass.enrolled.length} capacity={gymClass.capacity} waitlist={gymClass.waitlist.length} />
      <div className="card-actions">
        ...
        :isWaitlisted ? <><span className="waitlist-label">⏳ Waitlist #{waitPos}</span><Button type="button" visual="link" onClick={()=>leaveClass(gymClass.id,uid)}>Leave Waitlist</Button></>
          ...
          {selClasses.length===0
            ...
            : selClasses.map(c=>(
              <div key={c.id} className="cal-class-item">
                ...
                {c.trainerName && <span className="cal-class-trainer">👤 {c.trainerName}</span>}
                {c.time && <span className="cal-class-time">🕐 {formatTime(c.time)}</span>}
                <span className="cal-class-enrolled">👥 {c.enrolled.length} / {c.capacity} enrolled</span>
              </div>
            ))
          }
        </div>
...
function CardsPage({ setCurrentPage }) {
  ...
  const prevClasses = useRef(classes);

  // Sync membersTarget with latest class data
  useEffect(()=>{
    if(membersTarget){
      const latest = classes.find(c=>c.id===membersTarget.id);
      if(latest) setMembersTarget(latest);
    }
  },[classes]);

  // Toast on waitlist promotion (members only)
  useEffect(()=>{
    if(!user||isTrainer) return;
    classes.forEach(c=>{
      const prev=prevClasses.current.find(p=>p.id===c.id);
      if(prev&&prev.waitlist.includes(user.id)&&c.enrolled.includes(user.id))
        setToast(`🎉 You've been moved into ${c.title}!`);
    });
    prevClasses.current=classes;
  },[classes,user,isTrainer]);
  ...
  let visible=[...classes];
  visible = visible.filter(inThisWeek); // list view: this week only
  if(!isTrainer&&view==="mine") visible=visible.filter(c=>c.enrolled.includes(user.id)||c.waitlist.includes(user.id));
  ...
  return (
    <section className="cards-page">
      ...
      {isTrainer && calView && (
        <MonthCalendar
          filterFn={c => c.trainerName === user.username}
          emptyMsg="You have no classes scheduled this day."
        />
      )}
      ...
      {!isTrainer && view==="mine" && calView && (
        <MonthCalendar
          filterFn={c => c.enrolled.includes(user.id)}
          emptyMsg="No classes enrolled on this day."
        />
      )}
